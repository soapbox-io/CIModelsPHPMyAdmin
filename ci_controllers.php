...
if (isset($plugin_list)) {
    $plugin_list['ci_controllers'] = array(
        'text' => 'CI Controllers',
        'extension' => '.php.txt',
        'mime_type' => 'text/php',
        'options' => array(
    ...
    $plugin_list['ci_controllers']['options'][] =
    ...
    $GLOBALS['cfg']['Export']['ci_controllers_verbose'] = true;
    $GLOBALS['cfg']['Export']['ci_controllers_index'] = true;
    $GLOBALS['cfg']['Export']['ci_controllers_relation'] = true;
    ...
    function handleCodeIgniterPHPBody($db, $table, $crlf)
    {
        ...
        if ($result)
        {
            ...
            $lines[] = "require APPPATH.'/libraries/REST_Controller.php';\n";
            ...
            // insert class name
            $lines[] = "class ".toCamelCase( $table, true )." extends REST_Controller { \n";

            $lines[] = "   function __construct()";
            $lines[] = "   {";
            $lines[] = "        parent::__construct();";
            $lines[] = "        \$this->load->model('".toCamelCase( $table, true )."_model','',TRUE);";
            $lines[] = "   }";

            $lines[] = "\n";

            $lines[] = "   function put_put()";
            $lines[] = "   {";
            $lines[] = "        \$data = array(";
            $lines[] = "        // Example data - Replace with fields from the '".$table."' table.";
            $lines[] = "        //    'name' => \$this->put('name'),";
            $lines[] = "        //    'email' => \$this->put('email')";
            $lines[] = "        );";
            $lines[] = "\n";
            $lines[] = "        \$id = \$this->".toCamelCase( $table, true )."_model->save(\$data);";
            $lines[] = "\n";
            $lines[] = "        \$return = \$this->".toCamelCase( $table, true )."_model->get_by_id(\$id)->row();";
            $lines[] = "\n";
            $lines[] = "        if(\$return)";
            $lines[] = "        {";
            $lines[] = "               \$this->response(\$return, 200);";
            $lines[] = "        }";
            $lines[] = "        else";
            $lines[] = "        {";
            $lines[] = "               \$this->response(array('error' => 'Entity could not be created'), 404);";
            $lines[] = "        }";
            $lines[] = "   }";

            $lines[] = "\n";

            $lines[] = "   function post_post()";
            $lines[] = "   {";
            $lines[] = "        \$data = array(";
            $lines[] = "        // Example data - Replace with fields from the '".$table."' table.";
            $lines[] = "        //    'name' => \$this->post('name'),";
            $lines[] = "        //    'email' => \$this->post('email')";
            $lines[] = "        );";
            $lines[] = "\n";
            $lines[] = "        \$id = \$this->".toCamelCase( $table, true )."_model->update(\$this->post('id'),\$data);";
            $lines[] = "\n";
            $lines[] = "        \$return = \$this->".toCamelCase( $table, true )."_model->get_by_id(\$this->post('id'))->row();";
            $lines[] = "\n";
            $lines[] = "        if(\$return)";
            $lines[] = "        {";
            $lines[] = "               \$this->response(\$return, 200);";
            ...
            $lines[] = "   function delete_delete()";
            $lines[] = "   {";
            $lines[] = "        \$data = \$this->".toCamelCase( $table, true )."_model->delete(\$this->get('id'));";
            $lines[] = "\n";
            $lines[] = "        \$message = array(";
            $lines[] = "            'data' => \$data,";
            $lines[] = "            'message' => 'DELETED!'";
            $lines[] = "        );";
            $lines[] = "\n";
            $lines[] = "        if(\$data)";
            $lines[] = "        {";
            $lines[] = "               \$this->response(\$message, 200);";
            $lines[] = "        }";
            $lines[] = "        else";
            $lines[] = "        {";
            $lines[] = "               \$this->response(array('error' => 'Entity could not be created'), 404);";
            ...
            $lines[] = "   function get_get()";
            $lines[] = "   {";
            $lines[] = "        if(!\$this->get('id'))";
            $lines[] = "        {";
            $lines[] = "               \$this->response(NULL, 400);";
            $lines[] = "        }";
            $lines[] = "\n";
            $lines[] = "        \$data = \$this->".toCamelCase( $table, true )."_model->get_by_id(\$this->get('id'))->row();";
            $lines[] = "\n";
            $lines[] = "        if(\$data)";
            $lines[] = "        {";
            $lines[] = "               \$this->response(\$data, 200);";
            $lines[] = "        }";
            $lines[] = "        else";
            $lines[] = "        {";
            $lines[] = "               \$this->response(array('error' => 'Entity could not be found'), 404);";
            $lines[] = "        }";
            $lines[] = "   }";

            $lines[] = "\n";

            $lines[] = "   function getsome_get()";
            $lines[] = "   {";
            $lines[] = "        \$data = \$this->".toCamelCase( $table, true )."_model->get_paged_list(\$this->get('limit'),\$this->get('offset'))->result();";
            $lines[] = "\n";
            $lines[] = "        if(\$data)";
            $lines[] = "        {";
            $lines[] = "               \$this->response(\$data, 200);";
            ...
            $lines[] = "   function getall_get()";
            $lines[] = "   {";
            $lines[] = "        \$data = \$this->".toCamelCase( $table, true )."_model->list_all()->result();";
            $lines[] = "\n";
            $lines[] = "        if(\$data)";
            $lines[] = "        {";
            $lines[] = "               \$this->response(\$data, 200);";
            ...
            $filename = '../../htdocs/controllers/'. toCamelCase( $table, true ) . '.php';
            $content = implode("\n", $lines);
            $fp = fopen($filename, 'w');
        ...
        return '/htdocs/controllers/'. toCamelCase( $table, true ) . '.php   ';
